...
Задание засчитывается как сданное, если:
  ...
  * до **10-го октября** реализована загрузка логов в HDFS и их ротация (промежуточный дедлайн);
  * до **14-го октября** реализован расчет хотя бы _трех_ метрик первой группы и хотя бы _одной_ метрики второй группы;
    результаты расчетов доступны через HTTP API (финальный дедлайн);
  * в период **с 14 октября по 28 октября** ваше решение выдает по HTTP API корректные данные за период
...
Дополнительных попугаев в этом задании можно заработать следующими способами:
  ...
  * реализация одной дополнительной метрики первой группы и одной дополнительной метрики второй группы
    конвертируется в одного попугая; расчет засчитываеся, если ваше решение в течение 2-х недель выдает
    по HTTP API корректные данные за последнюю неделю;
  * если ваше решение корректно работает в течение 60 дней, то вы получаете одного попугая.
  * суммарно за это задание можно получить не более трех дополнительных попугаев.

## Процесс сдачи задания

_Информация появится в ближайшее время_.
...
**NB: До 1-го октября логи могут изменяться/удаляться в виду пусконаладочных работ**.
**С 1-го октября поток данных будет стабильный**.
...
Метрики второй группы:
  ...
  * `new_users` -- Количество новых пользователей. Пользователь считается новым в день X,
    если он посетил социальную сеть Zwitter в день X, но не посещал ее в течение последних двух недель (14 дней).
  * `lost_users` -- Количество потерянных пользоватеей. Пользователь считается потерянным в день X, если он _не_ посещал
    социальную сеть Zwitter в день X, а также в течение последних двух недель (14 дней), но посещал социальную сеть до этого.
  ...
  * `facebook_promo_conversion` -- Отношение количества пользователей, пришедших на социальную сеть из Facebook
  (первый переход в социальную сеть осуществлен с Facebook) и посетивших страницу `/promo` (необязательно в рамках одной сессии),
  к суммарному количеству пользователей, пришедших на социальную сеть из Facebook (дробное число).
...
## Дополнительные комментарии
  ...
  * Процесс ротации старых логов в HDFS совместите с загрузкой новых данных: удаляйте старые данные после успешной загрузки новых.
